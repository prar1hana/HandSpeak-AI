...
def capture_images(ges_name):
    ...
    for loop in listImage:
        while True:
            ...
            lo_h = cv2.getTrackbarPos("L - H", "Trackbars")
            lo_s = cv2.getTrackbarPos("L - S", "Trackbars")
            lo_v = cv2.getTrackbarPos("L - V", "Trackbars")
            up_h = cv2.getTrackbarPos("U - H", "Trackbars")
            up_s = cv2.getTrackbarPos("U - S", "Trackbars")
            up_v = cv2.getTrackbarPos("U - V", "Trackbars")

            img = cv2.rectangle(frame, (425, 100), (625, 300), (0, 255, 0), thickness=2, lineType=8, shift=0)

            lower_lim = np.array([lo_h, lo_s, lo_v])
            upper_lim = np.array([up_h, up_s, up_v])
            image_crop = img[102:298, 427:623]
            hsv = cv2.cvtColor(image_crop, cv2.COLOR_BGR2HSV)
            mask = cv2.inRange(hsv, lower_lim, upper_lim)

            result_image = cv2.bitwise_and(image_crop, image_crop, mask=mask)
